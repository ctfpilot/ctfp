...
module "kube-hetzner" {
  ...
  version = var.kube_hetzner_version # Not possible to make a variable when using Terraform - See https://github.com/hashicorp/terraform/issues/28912
  ...
  # source = "github.com/The0mikkel/terraform-hcloud-kube-hetzner?ref=2.18.0-fix"
  ...
  # * Your ssh public key
  ssh_public_key = base64decode(var.ssh_key_public_base64)
  ...
  ssh_private_key = base64decode(var.ssh_key_private_base64)
  ...
  network_region = var.network_zone
  ...
  control_plane_nodepools = [
    {
      ...
      server_type = var.control_plane_type_1,
      location    = var.region_1,
      labels = [
        "ressource-type=node",
        "node-type=control-plane",
        "control-plane=${var.region_1}",
        "node-pool=control-plane-${var.region_1}",
        "cluster.ctfpilot.com/node=control-plane",
      ],
      taints = [],
    ...
    },
    {
      ...
      server_type = var.control_plane_type_2,
      location    = var.region_2,
      labels = [
        "ressource-type=node",
        "node-type=control-plane",
        "control-plane=${var.region_2}",
        "node-pool=control-plane-${var.region_2}",
        "cluster.ctfpilot.com/node=control-plane",
      ],
      taints       = [],
    ...
    },
    {
      ...
      server_type = var.control_plane_type_3,
      location    = var.region_3,
      labels = [
        "ressource-type=node",
        "node-type=control-plane",
        "control-plane=${var.region_3}",
        "node-pool=control-plane-${var.region_3}",
        "cluster.ctfpilot.com/node=control-plane",
      ],
      taints       = [],
  ...
  agent_nodepools = [
    {
      name        = "agents-1",
      server_type = var.agent_type_1,
      location    = var.region_1,
      labels = [
        "ressource-type=node",
        "node-type=agent",
        "region=${var.region_1}",
        "node-pool=agents-${var.region_1}",
        "cluster.ctfpilot.com/node=agent",
      ],
      taints = [],
      count  = var.agent_count_1
      kubelet_args = [
    ...
    {
      name        = "agents-2",
      server_type = var.agent_type_2,
      location    = var.region_2,
      labels = [
        "ressource-type=node",
        "node-type=agent",
        "region=${var.region_2}",
        "node-pool=agents-${var.region_2}",
        "cluster.ctfpilot.com/node=agent",
      ],
      taints = [],
      count  = var.agent_count_2
      kubelet_args = [
        "kube-reserved=cpu=250m,memory=750Mi,ephemeral-storage=1Gi",
        "system-reserved=cpu=400m,memory=750Mi",
        "eviction-soft=memory.available<512Mi", # Recommend 1Gi for larger nodes
    ...
    {
      name        = "agents-3",
      server_type = var.agent_type_3,
      location    = var.region_3,
      labels = [
        "ressource-type=node",
        "node-type=agent",
        "region=${var.region_3}",
        "node-pool=agents-${var.region_3}",
        "cluster.ctfpilot.com/node=agent",
      ],
      taints = [],
      count  = var.agent_count_3
      kubelet_args = [
        "kube-reserved=cpu=250m,memory=750Mi,ephemeral-storage=1Gi",
        "system-reserved=cpu=400m,memory=750Mi",
        "eviction-soft=memory.available<512Mi", # Recommend 1Gi for larger nodes
    ...
    },
    {
      ...
      labels = [
        ...
        "region=${var.region_1}",
        "node-pool=challs",
        "cluster.ctfpilot.com/node=scaler"
      ]
      taints = [
  ...
  load_balancer_location = var.region_1
  ...
  # Example:
  traefik_additional_options = ["--api", "--api.dashboard=true", "--api.insecure=true"] # ["--log.level=DEBUG", "--tracing=true"]
  ...
  extra_firewall_rules = [
    {
      description     = "Allow Outbound SMTP Requests - Brevo"
      direction       = "out"
      protocol        = "tcp"
      port            = "587"
      source_ips      = []
      destination_ips = ["1.179.112.0/20", "172.246.240.0/20"]
    },
    {
      description     = "CTF - Allow all outbound connection"
      direction       = "out"
      protocol        = "tcp"
      port            = "1024-65535"
      source_ips      = []
